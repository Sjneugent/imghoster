...
class SqliteAdapter extends BaseAdapter {
  ...
  async init(config: string | Record<string, unknown>): Promise<this> {
    const dbPath = typeof config === 'string' ? config : (config.path as string);
    ...
    const imageCols = this.db.prepare("PRAGMA table_info(images)").all() as Array<{ name: string }>;
  ...
  async createUser(username: string, plainPassword: string, isAdmin = false, profile: UserProfile = {}): Promise<number> {
    ...
    ).run(username, email, realName, hash, isAdmin ? 1 : 0);
    return Number(result.lastInsertRowid);
  }

  async getUserByUsername(username: string): Promise<UserRow | undefined> {
    return this.getDb().prepare('SELECT * FROM users WHERE username = ?').get(username) as UserRow | undefined;
  }

  async getUserByEmail(email: string): Promise<UserRow | undefined> {
    return this.getDb().prepare('SELECT * FROM users WHERE LOWER(email) = LOWER(?)').get(email) as UserRow | undefined;
  }

  async getUserById(id: number): Promise<PublicUserRow | undefined> {
    return this.getDb().prepare(
      'SELECT id, username, email, real_name, is_admin, created_at FROM users WHERE id = ?'
  ...
  async createApiToken({ userId, tokenHash, label, expiresAt }: ApiTokenCreateData): Promise<number> {
    ...
    ).run(userId, tokenHash, label || null, expiresAt);
    return Number(result.lastInsertRowid);
  }
  ...
  async getActiveApiTokenByHash(tokenHash: string): Promise<ActiveApiTokenRow | undefined> {
    ...
    ).get(tokenHash) as ActiveApiTokenRow | undefined;
  }
  ...
  async upsertImageBlob(imageId: number, blobData: Buffer): Promise<void> {
    this.getDb().prepare(
      `INSERT INTO image_blobs (image_id, blob_data, blob_size) VALUES (?, ?, ?)
       ON CONFLICT(image_id) DO UPDATE SET blob_data = excluded.blob_data, blob_size = excluded.blob_size`
    ).run(imageId, blobData, blobData.length);
  }

  async getImageBlobByImageId(imageId: number): Promise<BlobRow | undefined> {
    return this.getDb().prepare(
      'SELECT image_id, blob_data, blob_size, created_at FROM image_blobs WHERE image_id = ?'
    ).get(imageId) as BlobRow | undefined;
  }

  async getImageBySlug(slug: string): Promise<ImageRow | undefined> {
    return this.getDb().prepare('SELECT * FROM images WHERE slug = ?').get(slug) as ImageRow | undefined;
  }

  async getImageById(id: number): Promise<ImageRow | undefined> {
    return this.getDb().prepare('SELECT * FROM images WHERE id = ?').get(id) as ImageRow | undefined;
  }
  ...
  async getImagesByIds(ids: number[]): Promise<ImageRow[]> {
    ...
    return this.getDb().prepare(`SELECT * FROM images WHERE id IN (${placeholders})`).all(...ids) as ImageRow[];
  }

  async checkDuplicateHash(fileHash: string | null): Promise<ImageRow | null | undefined> {
    if (!fileHash) return null;
    return this.getDb().prepare(
      'SELECT id, slug, original_name, user_id, created_at FROM images WHERE file_hash = ? LIMIT 1'
    ).get(fileHash) as ImageRow | undefined;
  }

  async getImagesByFileHash(fileHash: string | null): Promise<ImageRow[]> {
    if (!fileHash) return [];
    return this.getDb().prepare(
      'SELECT id, slug, original_name, user_id, created_at FROM images WHERE file_hash = ? ORDER BY created_at DESC'
    ).all(fileHash) as ImageRow[];
  }
    ...
    if (userId) {
      return this.getDb().prepare(
        `SELECT i.id, i.slug, i.original_name, i.created_at,
         ...
         FROM images i LEFT JOIN image_views v ON v.image_id = i.id
         WHERE i.user_id = ? GROUP BY i.id ORDER BY view_count DESC`
      ).all(userId) as ImageStatRow[];
    }
    return this.getDb().prepare(
      `SELECT i.id, i.slug, i.original_name, i.created_at, u.username,
       ...
       FROM images i JOIN users u ON u.id = i.user_id
       LEFT JOIN image_views v ON v.image_id = i.id
       GROUP BY i.id ORDER BY view_count DESC`
    ).all() as ImageStatRow[];
  }
  ...
  async getViewsOverTime(imageId: number | null, days = 30, userId: number | null = null): Promise<TimelineRow[]> {
    ...
    if (hasUserFilter) {
      if (hasImageFilter) {
        ...
        ).all(days, imageId, userId) as TimelineRow[];
      }
      return this.getDb().prepare(
        `SELECT date(v.viewed_at) AS day, COUNT(*) AS views
         FROM image_views v JOIN images i ON i.id = v.image_id
         ...
         GROUP BY day ORDER BY day`
      ).all(days, userId) as TimelineRow[];
    }
    ...
    if (hasImageFilter) {
      return this.getDb().prepare(base + ' AND image_id = ? GROUP BY day ORDER BY day').all(days, imageId) as TimelineRow[];
    }
    return this.getDb().prepare(base + ' GROUP BY day ORDER BY day').all(days) as TimelineRow[];
  }
  ...
  async exportData(): Promise<ExportData> {
    ...
    return {
      ...
      images: db.prepare('SELECT * FROM images').all() as ImageRow[],
      image_views: db.prepare('SELECT * FROM image_views').all() as ExportData['image_views'],
      image_blobs: db.prepare('SELECT * FROM image_blobs').all() as BlobRow[],
  ...
  async importData(data: ExportData): Promise<void> {
    ...
    const trx = db.transaction(() => {
      ...
      const insertUser = db.prepare(
        `INSERT INTO users (id, username, email, real_name, password_hash, is_admin, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)`
      );
      ...
      const insertImage = db.prepare(
        `INSERT INTO images (id, filename, original_name, slug, mime_type, size, storage_backend, file_hash, comment, tags, user_id, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
      );
      ...
      if (Array.isArray(data.image_blobs)) {
        const insertBlob = db.prepare(
          `INSERT INTO image_blobs (image_id, blob_data, blob_size, created_at) VALUES (?, ?, ?, ?)`
        );
      ...
      const insertView = db.prepare(
        `INSERT INTO image_views (id, image_id, viewed_at, ip_address, referrer) VALUES (?, ?, ?, ?, ?)`
      );
      ...
      const insertToken = db.prepare(
        `INSERT INTO api_tokens (id, user_id, token_hash, label, expires_at, last_used_at, revoked_at, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`
      );
  ...
  async createContentFlag({ imageId, flagType, reason, reporterName = null, reporterEmail = null, reporterCountry = null }: ContentFlagCreateData): Promise<number> {
    const result = this.getDb().prepare(
      `INSERT INTO content_flags (image_id, flag_type, reason, reporter_name, reporter_email, reporter_country) VALUES (?, ?, ?, ?, ?, ?)`
    ).run(imageId, flagType, reason, reporterName, reporterEmail, reporterCountry);
    return Number(result.lastInsertRowid);
  }
  ...
  async getContentFlag(flagId: number): Promise<ContentFlagRow | undefined> {
    return this.getDb().prepare(
      `SELECT f.*, i.slug, i.original_name, u.username
       ...
       WHERE f.id = ?`
    ).get(flagId) as ContentFlagRow | undefined;
  }
  ...
  async createFlagResolution({ flagId, adminId = null, action, notes = null, evidenceUrl = null }: FlagResolutionCreateData): Promise<number> {
    const result = this.getDb().prepare(
      `INSERT INTO flag_resolutions (flag_id, admin_id, action, notes, evidence_url) VALUES (?, ?, ?, ?, ?)`
    ).run(flagId, adminId, action, notes, evidenceUrl);
    return Number(result.lastInsertRowid);
  }
  ...
  async getFlagResolutions(flagId: number): Promise<unknown[]> {
    return this.getDb().prepare(
      `SELECT r.*, u.username as admin_username
       ...
       WHERE r.flag_id = ? ORDER BY r.resolved_at DESC`
    ).all(flagId);
  }
  ...
  async upsertImageThumbnail(imageId: number, thumbData: Buffer, width: number, height: number): Promise<void> {
    this.getDb().prepare(
      `INSERT INTO image_thumbnails (image_id, thumb_data, thumb_size, width, height) VALUES (?, ?, ?, ?, ?)
    ...
    ).run(imageId, thumbData, thumbData.length, width, height);
  }

  async getImageThumbnail(imageId: number): Promise<ThumbnailRow | undefined> {
    return this.getDb().prepare(
      'SELECT image_id, thumb_data, thumb_size, width, height FROM image_thumbnails WHERE image_id = ?'
    ).get(imageId) as ThumbnailRow | undefined;
  }
  ...
  async getAlbumById(id: number): Promise<AlbumRow | undefined> {
    return this.getDb().prepare('SELECT * FROM albums WHERE id = ?').get(id) as AlbumRow | undefined;
  }
  ...
  async listAlbumsByUser(userId: number): Promise<(AlbumRow & { image_count: number })[]> {
    ...
    ).all(userId) as (AlbumRow & { image_count: number })[];
  }
  ...
  async getAlbumImages(albumId: number): Promise<ImageWithStats[]> {
    return this.getDb().prepare(
      `SELECT i.*, ai.sort_order, COUNT(v.id) AS view_count
       ...
       LEFT JOIN image_views v ON v.image_id = i.id
       WHERE ai.album_id = ? GROUP BY i.id ORDER BY ai.sort_order`
    ).all(albumId) as ImageWithStats[];
  }
  ...
  async getExpiredImages(): Promise<ImageRow[]> {
    return this.getDb().prepare(
      `SELECT * FROM images WHERE expires_at IS NOT NULL AND datetime(expires_at) <= datetime('now')`
    ).all() as ImageRow[];
  }
  ...
  async getUserStorageUsed(userId: number): Promise<number> {
    const row = this.getDb().prepare(
      'SELECT COALESCE(SUM(size), 0) AS used FROM images WHERE user_id = ?'
  ...
  async getTotpSecret(userId: number): Promise<TotpSecretRow | undefined> {
    return this.getDb().prepare('SELECT * FROM totp_secrets WHERE user_id = ?').get(userId) as TotpSecretRow | undefined;
  }
