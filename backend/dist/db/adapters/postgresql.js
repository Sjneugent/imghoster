...
class PostgresAdapter extends BaseAdapter {
    ...
    async updateUserPassword(id, plainPassword) {
        const hash = await bcrypt.hash(plainPassword, SALT_ROUNDS);
        await this.getPool().query('UPDATE users SET password_hash = $1 WHERE id = $2', [hash, id]);
    }
    ...
    async revokeApiToken(userId, tokenId) {
        await this.getPool().query(`UPDATE api_tokens SET revoked_at = NOW() WHERE id = $1 AND user_id = $2 AND revoked_at IS NULL`, [tokenId, userId]);
    }
    async touchApiTokenUsage(tokenId) {
        await this.getPool().query('UPDATE api_tokens SET last_used_at = NOW() WHERE id = $1', [tokenId]);
    }
    // ── Image helpers ─────────────────────────────────────────────────────────
    ...
    async deleteImage(id) {
        await this.getPool().query('DELETE FROM images WHERE id = $1', [id]);
    }
    ...
    async updateFlagStatus(flagId, newStatus) {
        return this._queryOne('UPDATE content_flags SET status = $1 WHERE id = $2 RETURNING id', [newStatus, flagId]);
    }
    ...
    async updateAlbum(id, { name, description }) {
        await this.getPool().query('UPDATE albums SET name = $1, description = $2 WHERE id = $3', [name, description || null, id]);
    }
    async deleteAlbum(id) {
        await this.getPool().query('DELETE FROM albums WHERE id = $1', [id]);
    }
    ...
    async removeImageFromAlbum(albumId, imageId) {
        await this.getPool().query('DELETE FROM album_images WHERE album_id = $1 AND image_id = $2', [albumId, imageId]);
    }
    ...
    async updateImageVisibility(imageId, visibility) {
        await this.getPool().query('UPDATE images SET visibility = $1 WHERE id = $2', [visibility, imageId]);
    }
    // ── Expiration helpers ────────────────────────────────────────────────────
    ...
    async updateImageExpiration(imageId, expiresAt) {
        await this.getPool().query('UPDATE images SET expires_at = $1 WHERE id = $2', [expiresAt, imageId]);
    }
    // ── Quota helpers ─────────────────────────────────────────────────────────
    ...
    async setUserStorageQuota(userId, quotaBytes) {
        await this.getPool().query('UPDATE users SET storage_quota_bytes = $1 WHERE id = $2', [quotaBytes, userId]);
    }
    // ── TOTP helpers ──────────────────────────────────────────────────────────
    ...
    async enableTotp(userId) {
        await this.getPool().query('UPDATE totp_secrets SET enabled = 1 WHERE user_id = $1', [userId]);
    }
    async disableTotp(userId) {
        await this.getPool().query('DELETE FROM totp_secrets WHERE user_id = $1', [userId]);
    }
