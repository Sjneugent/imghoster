...
router.post('/', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Failed to create album', { error: (err as Error).message });
...
router.get('/', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Failed to list albums', { error: (err as Error).message });
...
router.get('/:id', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Failed to get album', { id: req.params.id, error: (err as Error).message });
...
router.patch('/:id', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    await updateAlbum(album.id, data as { name?: string; description?: string | null });
    res.json({ id: album.id, ...data });
  } catch (err) {
    logger.error('Failed to update album', { id: req.params.id, error: (err as Error).message });
...
router.delete('/:id', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Failed to delete album', { id: req.params.id, error: (err as Error).message });
...
router.post('/:id/images', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    const safeIds = (imageIds as unknown[]).map(Number).filter((id: number) => Number.isInteger(id) && id > 0);
    await addImagesToAlbum(album.id, safeIds);
    res.json({ message: `Added ${safeIds.length} image(s) to album.` });
  } catch (err) {
    logger.error('Failed to add images to album', { id: req.params.id, error: (err as Error).message });
...
router.delete('/:id/images/:imageId', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Failed to remove image from album', { error: (err as Error).message });
