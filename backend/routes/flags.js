...
import { createContentFlag, getContentFlag, listContentFlags, getFlagCountByStatus, updateFlagStatus, createFlagResolution, getFlagWithResolutions, getImageById, } from '../db/index.js';
...
router.post('/', async (req, res) => {
    try {
        ...
        logger.error('Error creating content flag:', { error: err.message });
...
router.get('/', requireAuth, async (req, res) => {
    try {
        ...
        logger.error('Error listing flags:', { error: err.message });
        res.status(500).json({ error: 'Internal server error' });
    }
});
router.get('/:flagId', requireAuth, async (req, res) => {
    try {
        ...
        logger.error('Error fetching flag:', { error: err.message });
        res.status(500).json({ error: 'Internal server error' });
    }
});
router.get('/image/:imageId', requireAuth, async (req, res) => {
    try {
        ...
        logger.error('Error fetching image flags:', { error: err.message });
        res.status(500).json({ error: 'Internal server error' });
    }
});
router.post('/:flagId/resolve', requireAuth, async (req, res) => {
    try {
        ...
        logger.error('Error resolving flag:', { error: err.message });
        res.status(500).json({ error: 'Internal server error' });
    }
});
router.patch('/:flagId/status', requireAuth, async (req, res) => {
    try {
        ...
        logger.error('Error updating flag status:', { error: err.message });
        res.status(500).json({ error: 'Internal server error' });
    }
});
