...
interface SchedulerStatus extends Omit<BackupConfig, 'lastBackupPath'> {
...
async function runBackup(): Promise<BackupResult | BackupFailure> {
  ...
  try {
    ...
    if (dbType === 'sqlite') {
      ...
      const adapter = getDB();
      const rawDb = (adapter as unknown as { getRawDB(): { backup(dest: string): Promise<void> } }).getRawDB();
...
function getSchedulerStatus(): SchedulerStatus {
  return {
...
function updateSchedulerConfig(updates: ConfigUpdate): SchedulerStatus {
  if (updates.enabled !== undefined) config.enabled = updates.enabled;
  if (updates.intervalMs !== undefined) config.intervalMs = Math.max(60000, Number(updates.intervalMs));
