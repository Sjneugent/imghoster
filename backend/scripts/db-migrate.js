 ...
 * Each adapter handles its own schema creation in init(), so this script is
 * primarily useful for verifying the schema and reinitialising if needed.
 *
 * Usage:
 *   node scripts/db-migrate.js [--status] [--reinit]
 *
 * Options:
 *   --status   Show current database status (tables, row counts)
 *   --reinit   Re-initialise the database (creates tables if missing)
 *
 * Examples:
 *   node scripts/db-migrate.js --status
 *   node scripts/db-migrate.js --reinit
