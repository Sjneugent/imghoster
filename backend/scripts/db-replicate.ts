...
import type BaseAdapter from '../db/BaseAdapter.js';
...
function createAdapter(dbType: string): BaseAdapter {
...
async function doExport(): Promise<void> {
  ...
  const data: ExportData = await adapter.exportData();
...
async function doImport(): Promise<void> {
  ...
  const data = JSON.parse(fs.readFileSync(inPath, 'utf8')) as ExportData;
...
async function doMigrate(): Promise<void> {
  ...
  const data: ExportData = await srcAdapter.exportData();
