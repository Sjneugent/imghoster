 ...
 */
async function createNamedProvider(name, overrides = {}) {
    ...
    switch (n) {
        ...
        default:
            throw new Error(`StorageProviderFactory: unknown provider "${name}". ` +
                'Valid values: local, s3, azure, gcs, dbblob, replicated.');
    }
}
async function createStorageProvider(overrides = {}) {
    // Determine the top-level provider name
    const rawProvider = String(overrides.provider
    ...
    if (providerName === 'replicated') {
        const rawList = String(overrides.providers
