...
class GcsProvider extends BaseStorageProvider {
    ...
    async loadPkg() {
        try {
            ...
            throw new Error('GcsProvider requires the "@google-cloud/storage" package. ' +
    ...
    async init(config) {
        ...
        if (!this.bucket)
            throw new Error('GcsProvider: STORAGE_GCS_BUCKET is required.');
        ...
        const storageOpts = {};
        if (projectId)
            storageOpts.projectId = projectId;
        if (keyFilename)
            storageOpts.keyFilename = keyFilename;
