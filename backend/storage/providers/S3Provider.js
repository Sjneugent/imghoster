...
class S3Provider extends BaseStorageProvider {
    ...
    async loadS3() {
        try {
            ...
            throw new Error('S3Provider requires the "@aws-sdk/client-s3" package. ' +
    ...
    async init(config) {
        ...
        if (!this.bucket)
            throw new Error('S3Provider: STORAGE_S3_BUCKET is required.');
    ...
    async getSignedUrl(key) {
        ...
        const { GetObjectCommand } = await this.loadS3();
        return getSignedUrl(this.client, new GetObjectCommand({ Bucket: this.bucket, Key: key }), { expiresIn: this.cdnTtl });
    }
    async close() {
